...
int t_i = blockIdx.x*blockDim.x + threadIdx.x;
int t_j = blockIdx.y*blockDim.y + threadIdx.y;
int tid   = gridDim.x * blockDim.x * t_j + t_i ;
cudaPres x=xMin+t_j*dx;
cudaPres y=yMin+t_i*dy;
...
cudaPres kX = KspaceFFT(t_j,nPointX, Lx);//kx[t_j];
cudaPres kY = KspaceFFT(t_i,nPointY, Ly);//ky[t_i];
//cudaPres kZ = KspaceFFT(t_k,nPointZ, Lz);//kz[t_k];
...
center = fp_tex2D(tex_psi2D,  t_i,   t_j);
right =  fp_tex2D(tex_psi2D,  t_i,   t_j+1);
left =   fp_tex2D(tex_psi2D,  t_i,   t_j-1);
up =     fp_tex2D(tex_psi2D,  t_i+1, t_j);
down =   fp_tex2D(tex_psi2D,  t_i-1, t_j);
...
fp_surf2DLayeredread(&center,surf_psi2D,    t_i,   t_j, int(0), cudaBoundaryModeZero);
fp_surf2DLayeredread(&up,    surf_psi2D,    t_i+1, t_j, int(0), cudaBoundaryModeZero);
fp_surf2DLayeredread(&down,  surf_psi2D,    t_i-1, t_j, int(0), cudaBoundaryModeZero);
fp_surf2DLayeredread(&right, surf_psi2D,    t_i,   t_j+1, int(0), cudaBoundaryModeZero);
fp_surf2DLayeredread(&left,  surf_psi2D,    t_i,   t_j-1, int(0), cudaBoundaryModeZero);
//fp_surf3Dread(&top,surf_psi, t_z+1,   t_y,   t_x, cudaBoundaryModeZero);
//fp_surf3Dread(&bottom,surf_psi, t_z-1,   t_y,   t_x, cudaBoundaryModeZero);
...
fp_surf2Dread(&center,surf_psi2DNL,    t_i,   t_j, cudaBoundaryModeZero);
fp_surf2Dread(&up,    surf_psi2DNL,    t_i+1, t_j,  cudaBoundaryModeZero);
fp_surf2Dread(&down,  surf_psi2DNL,    t_i-1, t_j,  cudaBoundaryModeZero);
fp_surf2Dread(&right, surf_psi2DNL,    t_i,   t_j+1,  cudaBoundaryModeZero);
fp_surf2Dread(&left,  surf_psi2DNL,    t_i,   t_j-1,  cudaBoundaryModeZero);
//fp_surf3Dread(&top,surf_psi, t_z+1,   t_y,   t_x, cudaBoundaryModeZero);
//fp_surf3Dread(&bottom,surf_psi, t_z-1,   t_y,   t_x, cudaBoundaryModeZero);
...
fp_surf2DLayeredwrite(  arr1, surf_psi2D,   t_i, t_j, int(0),  cudaBoundaryModeClamp);
// fp_surf3Dwrite(  arr2, surf_psi0OutImag,  t_x*sizeof(cudaPres), t_y, t_z,  cudaBoundaryModeClamp);
}
...
fp_surf2Dwrite(  arr1, surf_psi2DNL,   t_i, t_j,  cudaBoundaryModeClamp);
// fp_surf3Dwrite(  arr2, surf_psi0OutImag,  t_x*sizeof(cudaPres), t_y, t_z,  cudaBoundaryModeClamp);
}